...
#ifdef _COMM
	//Comm Toolbar
	if (!m_wndCommBar.Create(this, WS_CHILD | WS_VISIBLE | CBRS_TOP |
			 CBRS_TOOLTIPS | CBRS_FLYBY | CBRS_SIZE_DYNAMIC ) ||
		!m_wndCommBar.LoadToolBar(IDR_COMM) )
	{
		TRACE0("Failed to create Comm bar\n");
		return -1;      // fail to create
	}
	m_wndCommBar.SetWindowText(_T("Communication"));
	m_wndCommBar.EnableDocking(CBRS_ALIGN_ANY);
	DockControlBar(&m_wndCommBar,AFX_IDW_DOCKBAR_RIGHT);
//	DockControlBarBottomOf(&m_wndToolBar, &m_wndCommBar);

	// Create the COMMCTRL OLE Control.
...
//m_dlgCommData.Create(IDD_COMM_DATA, this);
//m_dlgCommData.ShowWindow(SW_SHOWNORMAL);
#endif	//_COMMDLG
#endif	//_COMM
...
void CMainFrame::ShowData(VARIANT pchData)
{
	CString str;
	int nCharIndex;
	int nBackcount = 0;
...
#ifdef	_COMMDLG
	if(m_dlgCommData.IsWindowEnabled())
	{
//Works only when dialog is active
//otherwise it gives error
		int len = m_dlgCommData.m_edit.GetWindowTextLength();
		if (len >= 16384)
		{
			m_dlgCommData.m_edit.GetWindowText(str);
			m_dlgCommData.m_edit.SetWindowText(str.GetBuffer(len)+4097);
			len -= 4097;
		}
		...
		if (nBackcount>len)
			nBackcount = len;
		m_dlgCommData.m_edit.SetSel(len-nBackcount,len);

		// Add filtered data to edit control.
		m_dlgCommData.m_edit.ReplaceSel(str);
	}
#endif	//_COMMDLG
...
//str contains the purified data received
//AfxMessageBox(str);
	DataExtractor(str);
}
...
void CMainFrame::Dialer()
{
	OnCommPort(ID_COMM_DIAL);
}

void CMainFrame::DataExtractor(CString str)
{
//     Stringer(szBufIn, szBuf);
//     if(lstrlen(szBuf))
//     { lstrcpy(szBufIn, szBuf);
//       InvalidateRect(hWnd, NULL, TRUE);//TO DISPLAY LINE AT A TIME
//       Feeder(szBuf);
//     }
...
BOOL CMainFrame::Stringer(char *szIn, char *szOut)
//BOOL CMainFrame::Stringer(CString& szIn, CString& szOut)
{
  static char szFIFO[128];
  int	      nCRPosR;
  int	      nCRPosL;

  szOut[0] = '\0';
  lstrcat(szFIFO, szIn);
  while(strchr(szFIFO, '\r'))               //CHECK IF CR CHARACTER IS PRESENT
  { int i,j=0;
    nCRPosR = lstrlen(strchr(szFIFO, '\r'));//RIGHT POSITION OF CR CHARACTER
    nCRPosL = lstrlen(szFIFO) - nCRPosR+1; 	//LEFT POSITION OF CR CHARACTER

    for(i=0; i<nCRPosL; i++)                //COPYING CHARACTERS TILL LF
      szOut[i] = szFIFO[i];
//    if(CommParas.bCrLfIn)                   //IF CR->CR/LF IS TRUE
//    { szOut[nCRPosL] = '\n';
//      szOut[nCRPosL+1] = '\0';
//    }
//    else
//      szOut[nCRPosL] = '\0';
    ...
    for(i=nCRPosL; i<strlen(szFIFO); i++)   //SHIFTING REMAINING CHARACTERS
      szFIFO[j++] = szFIFO[i];
    szFIFO[j] = '\0';
  }
  return(TRUE);
...
void CMainFrame::Feeder(CString sEntry)
{
	CString	sFrom;
	CString sTo;
	CString	sTime;
	...
	if(sEntry.Left(4) != "5 09") return;
	if(sEntry.Mid(58, 12) == "EV_CP_CC_ANS")
	{
		sFrom	= sEntry.Mid(13, 3);
		sTo		= sEntry.Mid(17, 3);
		t		= CTime::GetCurrentTime();
		sTime	= t.Format("%d/%m/%y %H:%M");
	}


	if(sEntry.Mid(58, 12) == "EV_CP_CC_RLS")
	{
		sFrom	= sEntry.Mid(17, 3);
		sTo		= sEntry.Mid(13, 3);
		t		= CTime::GetCurrentTime();
		sTime	= t.Format("%d/%m/%y %H:%M");
	}
